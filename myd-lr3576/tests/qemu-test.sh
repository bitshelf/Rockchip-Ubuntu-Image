...
init_test_env() {
    ...
    test_pass "qemu-aarch64-static found"

    # Check binfmt support
    if [[ -f /proc/sys/fs/binfmt_misc/qemu-aarch64 ]]; then
        test_pass "binfmt_misc qemu-aarch64 registered"
    else
        test_warn "binfmt_misc qemu-aarch64 not registered, attempting auto-register"
...
test_image_identity() {
    ...
    if [[ -f "${STAGING}/etc/os-release" ]]; then
        local os_name
        os_name=$(sudo chroot "${STAGING}" /usr/bin/qemu-aarch64-static /bin/bash -c \
        ...
        local arch
        arch=$(sudo chroot "${STAGING}" /usr/bin/qemu-aarch64-static /bin/bash -c \
...
test_package_integrity() {
    ...
    local dpkg_verify
    dpkg_verify=$(sudo chroot "${STAGING}" /usr/bin/qemu-aarch64-static /bin/bash -c \
    ...
    # apt-get check
    if sudo chroot "${STAGING}" /usr/bin/qemu-aarch64-static /usr/bin/apt-get check >/dev/null 2>&1; then
    ...
    # dpkg --audit
    local audit
    audit=$(sudo chroot "${STAGING}" /usr/bin/qemu-aarch64-static /bin/bash -c \
...
test_excluded_packages() {
    ...
    local found
    found=$(sudo chroot "${STAGING}" /usr/bin/qemu-aarch64-static /bin/bash -c \
    ...
    for pkg in systemd apt dpkg linux-firmware network-manager; do
        if sudo chroot "${STAGING}" /usr/bin/qemu-aarch64-static /bin/bash -c \
...
test_systemd() {
    ...
    for timer in apt-daily.timer apt-daily-upgrade.timer motd-news.timer; do
        local status
        status=$(sudo chroot "${STAGING}" /usr/bin/qemu-aarch64-static /bin/bash -c \
...
test_ubuntu_official() {
    ...
    if sudo chroot "${STAGING}" /usr/bin/qemu-aarch64-static /bin/bash -c \
        ...
        sudo chroot "${STAGING}" /usr/bin/qemu-aarch64-static /bin/bash -c \
